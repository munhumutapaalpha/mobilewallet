    ...
    NRS = (function (NRS, $, undefined) {
        ...
        function isResponseReadyForSigning(response, data, requestType) {
            return response.unsignedTransactionBytes && !data.doNotSign &&
                ...
                !response.binderRateMTAPerFXT && !data.calculateFee && requestType !== "getTransactionBytes";
        }
        ...
        function performDataConversion(data) {
            ...
            try {
                ...
                for (let i = 0; i < NXT_FIELDS.length; i++) {
                    ...
                    let mtaField = NXT_FIELDS[i][1];
                    if (field in data) {
                        data[mtaField] = NRS.convertToMTA(data[field]);
                        delete data[field];
                    }
                }
        ...
        NRS.verifyTransactionBytes = function (transactionBytes, requestType, data, attachment, isVerifyECBlock) {
            try {
                ...
                if (transaction.amountMTA !== data.amountMTA && !(requestType === "exchangeCoins" && transaction.amountMTA === "0")) {
                    return {fail: true, param: "amountMTA", actual: transaction.amountMTA, expected: data.amountMTA};
                }
        ...
        NRS.verifyTransactionTypes = function (byteArray, transaction, requestType, data, pos, attachment) {
            ...
            switch (requestType) {
                ...
                case "sellAlias":
                    ...
                    if (transaction.priceMTA !== String(data.priceMTA)) {
                        return verificationFailed('priceMTA', data.priceMTA, transaction.priceMTA);
                    }
                    break;
                case "buyAlias":
                ...
                case "placeBidOrder":
                    ...
                    pos += 8;
                    verificationResult = verifyFields(transaction, data, ['asset', 'priceMTAPerShare', 'quantityQNT']);
                    if (verificationResult.fail) {
                        return verificationResult;
                    }
                    break;
                ...
                case "dividendPayment":
                    ...
                    pos += 8;
                    verificationResult = verifyFields(transaction, data, ['holding', 'holdingType', 'asset', 'height', 'amountMTAPerShare']);
                    if (verificationResult.fail) {
                        return verificationResult;
                    }
                    break;
                ...
                case "dgsListing":
                    ...
                    transaction.priceMTA = String(converters.byteArrayToBigInteger(byteArray, pos));
                    pos += 8;
                    verificationResult = verifyFields(transaction, data, ['name', 'description', 'tags', 'quantity', 'priceMTA']);
                    if (verificationResult.fail) {
                        return verificationResult;
                    }
                    break;
                ...
                case "dgsPriceChange":
                    ...
                    if (transaction.priceMTA !== data.priceMTA) {
                        return verificationFailed('priceMTA', data.priceMTA, transaction.priceMTA);
                    }
                    break;
                ...
                case "dgsPurchase":
                    ...
                    pos += 4;
                    verificationResult = verifyFields(transaction, data, ['goods', 'quantity', 'priceMTA', 'deliveryDeadlineTimestamp']);
                    if (verificationResult.fail) {
                        return verificationResult;
                    }
                    break;
                case "dgsDelivery":
                    ...
                    verificationResult = verifyFields(transaction, data, ['purchase', 'goodsData', 'goodsNonce', 'discountMTA']);
                    if (verificationResult.fail) {
                        return verificationResult;
                    }
                    break;
                ...
                case "dgsRefund":
                    ...
                    if (transaction.refundMTA !== data.refundMTA) {
                        return verificationFailed('refundMTA', data.refundMTA, transaction.refundMTA);
                    }
                    break;
                case "leaseBalance":
                ...
                case "issueCurrency":
                    ...
                    fields = ['issuanceHeight', 'reserveSupplyQNT', 'minReservePerUnitMTA', 'minDifficulty', 'maxDifficulty', 'ruleset', 'algorithm', 'decimals'];
                ...
                case "currencyReserveIncrease":
                    ...
                    pos += 8;
                    verificationResult = verifyFields(transaction, data, ['currency', 'amountPerUnitMTA']);
                    if (verificationResult.fail) {
                        return verificationResult;
                    }
                    break;
                ...
                case "publishExchangeOffer":
                    ...
                    pos += 4;
                    fields = ['currency', 'buyRateMTAPerUnit', 'sellRateMTAPerUnit', 'totalBuyLimitQNT',
                ...
                case "currencyBuy":
                    ...
                    transaction.unitsQNT = String(converters.byteArrayToBigInteger(byteArray, pos));
                    pos += 8;
                    verificationResult = verifyFields(transaction, data, ['currency', 'rateMTAPerUnit', 'unitsQNT']);
                    if (verificationResult.fail) {
                        return verificationResult;
                    }
                    break;
                ...
                case "exchangeCoins":
                    ...
                    pos += 8;
                    let priceMTAPerCoin = String(converters.byteArrayToBigInteger(byteArray, pos));
                    if (priceMTAPerCoin !== data.priceMTAPerCoin) {
                        return {
                            fail: true,
                            param: requestType,
                            actual: priceMTAPerCoin,
                            expected: data.priceMTAPerCoin
                        };
                    }
                    pos += 8;
                    break;
