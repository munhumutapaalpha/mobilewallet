...
let ledgerwallet = function() {
    ...
    const MunhumutapaResponseRCValues = {
        SUCCESS: 0,
        WRONG_RET_SIZE: 129,
    ...
    const MUNHU_SIG = [0xba, 0xbe, 0x00];

    const MunhumutapaLedgerAppResponse = function MunhumutapaLedgerAppResponse(rc, data) {
        this.rc = rc;
        this.data = data;
    };
    ...
    async function connectWebUsb() {
        ...
        try {
            ...
            if ("BOLOS" === name) {
                ...
                await transport.close();
                return null;
            } else if ("Munhumutapa" !== name) {
                console.log("App is not munhumutapa its ", name);
                notifyCallbacks(LEDGER_STATE.DEVICE_CONNECTED_APP_NOT_OPEN);
                await transport.close();
                return null;
            }
    ...
    async function getVersion() {
        ...
        if (7 !== response.length) {
            return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.WRONG_RET_SIZE, response);
        }
        ...
        const isSigCorrect = MUNHU_SIG[0] === sig1 && MUNHU_SIG[1] === sig2 && MUNHU_SIG[2] === sig3;
        return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.SUCCESS, {
    ...
    async function getPublicKeyAndChainCode(path, isGetPublicKeyAndChainCode) {
        ...
        if (!pathData.isValid) {
            return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.BAD_PARAM);
        }
        ...
        if (result.length === 0) {
            return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.RET_WRONG_LENGTH, result);
        }

        if (result[0] !== 0) {
            return new MunhumutapaLedgerAppResponse(result[0]);
        }
    ...
    async function signToken(path, timestamp, message) {
        ...
        while (pos < messageBuffer.length) {
            ...
            if (result[0] !== 0) {
                return new MunhumutapaLedgerAppResponse(result[0], result);
            }
            pos = endPos;
        }
        ...
        if (result.length === 101 && result[0] === 0) {
            return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.SUCCESS, chunkedBase32BufferEncode(result.slice(1, 101)));
        }
        return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.RET_WRONG_LENGTH, result);
    }
    ...
    async function loadTxn(txnArray) {
        ...
        if ((txnBuffer.length & 0b1100000000000000) > 0) {
            return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.RET_BUFFER_TOO_BIG);
        }
        ...
        while (pos < txnBuffer.length) {
            ...
            if (endPos === txnBuffer.length) { //Are all the bytes sent?
                ...
                if (result[1] === 1) { //RET_VAL_REJECT_BUTTON todo maybe this value will change some day, find a way to sync these
                    return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.SUCCESS, false);
                }

                if (result[1] === 8) { //RET_VAL_PARSE_BUFFER_FINISHED todo maybe this value will change some day, find a way to sync these
                    return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.SUCCESS, true);
                }
                return new MunhumutapaLedgerAppResponse(result[1]);
            }
            if (result.length < 1) {
                return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.RET_WRONG_LENGTH, result);
            }
    ...
    async function signTxn(path) {
        ...
        if (result.length === 64 + 1 && result[0] === 0) {
            return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.SUCCESS, result.slice(1, 64 + 1));
        }
        return new MunhumutapaLedgerAppResponse(MunhumutapaResponseRCValues.RET_WRONG_LENGTH, result);
    }
